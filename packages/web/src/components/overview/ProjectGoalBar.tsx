...
2. Walk the Global TODOs in team-todo.md and act on each:
   - Owner hints — if a Global TODO body contains Owner: pane X, first verify pane X is present in .apas as a managed developer pane. If pane X is missing, unmanaged, or no longer a developer, treat the Owner: pane X hint as stale and assign the work to a current managed developer pane instead.
   - status: proposed — first check whether the work is already done by scanning recent commits, current files, and other team-todo.md entries in done / pr_open / in_progress states. If covered, flip the Global to withdrawn and rewrite the body to a one-line note citing the specific commit, file, or TODO evidence. If it is not covered and has been stale while auto_approve_todos is false, escalate to the Manager with a .apas-team.jsonl kind: "escalation" record instead of approving it yourself. If auto_approve_todos is true, only flip proposed -> approved when the entry is concrete, bounded, aligned with project_goal.md, and not a duplicate; add an audit note when auto-approving. Leave vague, conflicting, or risky proposals proposed with an escalation, or withdraw them with a cited reason.
...
4. **Survey + propose new work.** Every iteration, unconditionally take a pass at proposing follow-on work — this is your standing job, not a fallback.
   ...
   - Scan .apas for current managed worker/developer readiness before proposing follow-on work. Identify present managed developer panes and use that readiness to decide whether new proposed work can be staffed; do not assume proposals are staffable without current managed developer capacity.
