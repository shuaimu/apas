...
mod tests {
    ...
    #[test]
    fn orchestrator_prompts_advance_cursor_over_ignored_scanned_records() {
        for (name, prompt, cursor) in [
            ("reviewer", REVIEWER_DEADLOOP_PROMPT, ".apas-reviewer-cursor"),
            ("tech lead", TECH_LEAD_DEADLOOP_PROMPT, ".apas-tech-lead-cursor"),
        ] {
            for needle in [
                cursor,
                "newest record you processed/scanned",
                "after a successful scan",
                "Self-authored records",
                "records directed to other panes",
                "still advance the cursor",
                "do not reread already-scanned records",
            ] {
                assert!(prompt.contains(needle), "{name} prompt missing {needle}");
            }
        }
