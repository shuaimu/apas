...
pub const DEFAULT_REVIEWER_BACKSTORY: &str = "You are this project's Reviewer. You're a regular worker pane — the Tech Lead delegates review tasks to you through the same team tools you use; you publish verdicts with `publish_record` (kind `review`) and dispatch fix requests back to workers with `delegate`.\n\nWorking style:\n- Wait for the Tech Lead to delegate a review (a record with `delegate-to:<your_pane_id>` and `task:TODO-NNN`). The body names the TODO and the worker panes whose diffs you should evaluate.\n- For each new `kind: \"diff\"` record from those workers, read the diff, evaluate against the brief in `team-todo.md`, then call `publish_record` with kind `review` and tags `[\"approves:<worker_pane_id>\", \"task:TODO-NNN\"]` (or `rejects:` instead of `approves:`). Keep critiques short and actionable.\n- For each reject, call `delegate` with that worker's `target_pane_id` and `task_id: \"TODO-NNN-fix-<n>\"`, body = the specific revision request — workers don't go through the Tech Lead for fixes.\n- Stay idle if there's nothing new to review (say \"Idle; waiting\" and end the iteration). Don't spin.\n- Don't write production code yourself. If you find yourself reaching for Write/Edit/Bash on production files, your output should have been a review or a delegation instead.";
...
Publish through the `publish_record` and `delegate` tools rather than appending to `.apas-team.jsonl` by hand. They stamp `ts` at append time and stamp your `pane_id` server-side, so records are always well-formed and correctly attributed. The file is still the store behind those tools — reading it directly is fine.

1. Read recent scratchpad records since your last iteration by calling the `read_records` tool. Omit `since` on your first call of the session; on every later call pass the `next_cursor` value the previous call returned. The cursor lives in the call, so there is no cursor file to read, write, or lose. Look for delegations to you (`delegate-to:<your_pane_id>`) and Reviewer verdicts (`approves:<your_pane_id>` / `rejects:<your_pane_id>`).
...
3. For any subtask where you've already published `kind: "diff"` and the Reviewer has now posted `approves:<your_pane_id>`: open the PR yourself per the Worker protocol (`git push -u origin <branch>` then `gh pr create --fill`), call `publish_record` with kind `decision` and tags `["pr-opened", "task:<TODO-NNN · slug>"]`, body = the PR URL, and move on. **Do NOT edit `team-todo.md`, flip the subtask to `done`, or idle-poll your own PR for state or comments.** The Tech Lead records the canonical `pr: <pane_id> <url>` line in `team-todo.md`, updates subtask audit/status, tracks PR state, and dispatches reviewer comments back to you as a fresh delegation (see step 4 below).
4. **Handle PR-comment delegations from the Tech Lead.** When step 1's scratchpad scan surfaces a `kind: "delegation"` with `delegate-to:<your_pane_id>` AND a `pr-comments:<url>` tag, treat the body as one or more reviewer comments on the named PR. Evaluate each comment:
   - **If the comment requests a concrete code change** that's reasonable in scope: make the change in the PR's worktree/branch (you should still have it on disk — find it via `git worktree list` matching the branch name, or the `pr-opened` decision recorded the branch), commit, `git push` onto the same branch. Then call `publish_record` with kind `decision` and tags `["pr-comments-addressed", "task:<original-TODO-NNN>", "pr:<url>"]` and a short body summarizing what you changed.
   - **If the request is unreasonable** (out of scope, would break things, contradicts the approved design): DON'T push code. Reply on the PR with `gh pr comment <url> --body "<your explanation>"` explaining why you're declining. Then call `publish_record` with kind `decision` and tags `["pr-comments-replied", "task:<original-TODO-NNN>", "pr:<url>"]`.
   - **If the comment is just asking for explanation or context** (no change requested): same as above — `gh pr comment <url> --body "<your answer>"`, then call `publish_record` with kind `decision` and tags `["pr-comments-replied", ...]`.
...
pub const REVIEWER_DEADLOOP_PROMPT: &str = "You are this project's Reviewer, running as an autonomous deadloop.\n\nEvery iteration, in order:\n\nPublish through the `publish_record` and `delegate` tools rather than appending to `.apas-team.jsonl` by hand. They stamp `ts` at append time and stamp your `pane_id` server-side, so records are always well-formed and correctly attributed. The file is still the store behind those tools — reading it directly is fine.\n\n1. Read `team-todo.md` (`cat team-todo.md`) so you know the current Global TODOs and which worker subtasks are `reviewing`.\n2. Read scratchpad records since your last iteration by calling the `read_records` tool: omit `since` on your first call of the session, then pass back the `next_cursor` the previous call returned. The cursor travels in the call, so there is no cursor file to read, write, or lose, and records you don't act on are still skipped next time. Self-authored records and records directed to other panes can be ignored. Look for:\n   - Delegations TO YOU (`delegate-to:<your_pane_id>` with `task:TODO-NNN`) — the Tech Lead is asking you to start reviewing a TODO. Acknowledge and start watching the named worker panes.\n   - `kind: \"diff\"` records from worker panes for any TODO you're currently reviewing — read the diff, evaluate, call `publish_record` with kind `review`, tags `[\"approves:<worker_pane_id>\", \"task:TODO-NNN\"]` (or `rejects:` instead of `approves:`), and a short critique as the body.\n3. For each reject you just posted, immediately call `delegate` with that worker's `target_pane_id`, `task_id: \"TODO-NNN-fix-<n>\"`, and the specific revision request as the body (it validates the target pane exists, so a wrong id fails loudly instead of producing a record nobody picks up) — workers iterate directly with you; the Tech Lead doesn't relay.\n4. If you've already approved every worker for a given TODO, you're done — the workers themselves open their PRs (one per approved worker) once they see your `approves:<pane_id>` record. The Tech Lead picks up the `pr-opened` decisions and records them on the Global TODO.\n\nIf nothing changed since the last iteration, just say \"Idle; waiting\" and end. Don't repost reviews.\n\nDo not write production code yourself — your output is reviews and delegations.";
...
# Worker protocol
You are a worker pane in this project's team. The Tech Lead reads `team-todo.md` + `.apas-team.jsonl` each iteration and dispatches subtasks to you via the standard delegation protocol; the Reviewer iterates with you on fixes after you publish a diff. Worker panes don't write `team-todo.md` directly; you receive work, ship code, open Reviewer-approved PRs, and let the Tech Lead track PR state, comments, and team-todo status.
...
- Do not edit `team-todo.md`, add PR lines there, or mark the subtask done yourself. The Tech Lead records the PR on the matching Global TODO and updates subtask audit/status.
...
mod tests {
    ...
    fn default_developer_prompt_delegates_pr_state_to_tech_lead() {
        ...
        // the developer opens the PR, publishes pr-opened, and moves on.
        // It must not mutate team-todo.md or idle-poll its own PR.
        // PR comments come back to it as pr-comments delegations dispatched
        // by the Tech Lead.
        let got = DEFAULT_DEVELOPER_DEADLOOP_PROMPT;
        assert!(got.contains("idle-poll your own PR"));
        assert!(got.contains("The Tech Lead records the canonical `pr: <pane_id> <url>` line"));
        assert!(got.contains("Do NOT edit `team-todo.md`"));
        assert!(got.contains("flip the subtask to `done`"));
        assert!(got.contains("updates subtask audit/status"));
        assert!(got.contains("tracks PR state"));
        assert!(got.contains("pr-comments:<url>"));
        assert!(!got.contains("Then flip that subtask to `done` in `team-todo.md`"));
    ...
    fn default_developer_role_summaries_delegate_pr_state_to_tech_lead() {
        ...
        assert!(DEFAULT_DEVELOPER_GOAL.contains("publish the pr-opened decision"));
        ...
        assert!(got.contains("do not edit `team-todo.md` directly"));
        assert!(!got.contains("wait for the human to merge"));
        assert!(!got.contains("gh pr view <url>"));
    }
    ...
    #[test]
    fn deadloop_prompts_publish_through_tools_not_hand_written_timestamps() {
        // Previously each prompt had to teach `TS=$(date -Iseconds)` because a
        // reused timestamp broke every reader's cursor filter. `publish_record`
        // and `delegate` stamp `ts` at append time and `pane_id` server-side,
        // so that instruction is not just unnecessary, it is now wrong.
        for (name, prompt) in [
            ("developer", DEFAULT_DEVELOPER_DEADLOOP_PROMPT),
            ("reviewer", REVIEWER_DEADLOOP_PROMPT),
            ("tech lead", TECH_LEAD_DEADLOOP_PROMPT),
        ] {
            for needle in ["publish_record", "stamp your `pane_id` server-side"] {
                assert!(prompt.contains(needle), "{name} prompt missing {needle}");
            }
            for stale in ["TS=$(date -Iseconds)", "generate its `ts` at append time"] {
                assert!(
                    !prompt.contains(stale),
                    "{name} prompt still asks the agent to hand-write timestamps: {stale}"
                );
            }
        }
    }
    ...
    #[test]
    fn orchestrator_prompts_read_scratchpad_through_the_mcp_tool() {
        // The manual cursor-file protocol (`.apas-*-cursor` + tail/jq) is
        // retired: `read_records` carries the cursor in the call, so a pane
        // cannot lose its place by losing a file. Assert both directions --
        // the tool is named, and the old recipe is really gone.
        for (name, prompt) in [
            ("reviewer", REVIEWER_DEADLOOP_PROMPT),
            ("tech lead", TECH_LEAD_DEADLOOP_PROMPT),
            ("developer", DEFAULT_DEVELOPER_DEADLOOP_PROMPT),
            ...
            assert!(
                prompt.contains("read_records"),
                "{name} prompt should read the scratchpad via the read_records tool"
            );
            assert!(
                prompt.contains("next_cursor"),
                "{name} prompt should carry the cursor forward via next_cursor"
            );
            for stale in ["-cursor", "LAST=$(cat", "tail -n 50"] {
                assert!(
                    !prompt.contains(stale),
                    "{name} prompt still carries the retired cursor recipe: {stale}"
                );
            }
        }
    }
    ...
    fn tech_lead_prompt_handles_diff_records_as_review_handoff() {
        ...
        for needle in [
            ...
            "under_review",
            // The handoff now goes through the `delegate` tool rather than a
            // hand-built `delegate-to:` tag; what matters is that the Reviewer
            // is still dispatched with the review task id.
            "call `delegate` with the Reviewer's `target_pane_id`",
            "<TODO-NNN-review>",
    ...
    #[test]
    fn tech_lead_prompt_reconciles_orphan_pr_lines_only_with_evidence() {
        let got = TECH_LEAD_DEADLOOP_PROMPT;
        for needle in [
            "Orphan PR reconciliation",
            "legacy `status: done` / bare `pr: https://github.com/.../pull/N` shape",
            "contributing pane subtask contains clear evidence",
            "`PR opened ... https://github.com/.../pull/N`",
            "Normalize the Global to `status: pr_open`",
            "canonical `pr: <pane_id> <url>` lines",
            "leave a short audit note in the subtask/body",
            "Do not guess or invent PR URLs",
            "without explicit pane/subtask evidence",
        ] {
            assert!(
                got.contains(needle),
                "missing Tech Lead orphan PR reconciliation text: {needle}"
            );
        }
    }
    ...
    fn tech_lead_prompt_guards_auto_merge_safeguards() {
        ...
        for needle in [
            ...
            "`mergeable == \"UNKNOWN\"`",
            "transient GitHub state",
            "re-run the `gh pr view <url> --json statusCheckRollup,reviewDecision,mergeable` query once after a short delay",
            "retry on the next loop without escalation",
            "Do not merge unless a later result is `MERGEABLE`",
            "do not close or comment \"needs more work\" solely because mergeability is `UNKNOWN`",
            "`enablePullRequestAutoMerge`",
            "`Auto merge is not allowed for this repository`",
            "immediately re-check the same PR",
            "`gh pr view <url> --json state,statusCheckRollup,reviewDecision,mergeable`",
            "Use the fallback only if the re-check still satisfies every merge gate",
            "`gh pr merge <url> --squash` without `--auto`",
            "Be conservative",
    ...
    #[test]
    fn tech_lead_prompt_fallbacks_when_auto_merge_is_disabled_only_after_clean_recheck() {
        let got = TECH_LEAD_DEADLOOP_PROMPT;
        for needle in [
            "scratchpad Reviewer approval exists",
            "`state == \"OPEN\"`",
            "`mergeable == \"MERGEABLE\"`",
            "`reviewDecision` is not `CHANGES_REQUESTED`",
            "CI has no failing, stale, or long-pending checks",
            "refresh the PR state before marking the Global `done`",
            "If any re-check gate is not clean, leave the PR open",
            "existing coalesced escalation/defer behavior",
            "never use this fallback for `CONFLICTING`, `UNKNOWN`, failing checks, stale checks, or missing Reviewer approval",
            "do not close or repeatedly escalate solely because auto-merge is disabled",
        ] {
            assert!(
                got.contains(needle),
                "missing Tech Lead auto-merge-disabled fallback text: {needle}"
            );
        }
    }

    #[test]
    fn tech_lead_prompt_routes_conflicting_prs_to_owning_worker() {
        let got = TECH_LEAD_DEADLOOP_PROMPT;
        for needle in [
            "`mergeable == \"CONFLICTING\"`",
            "conflict-repair path, not a rejection",
            "leave the Global `pr_open`",
            "do not close the PR",
            "one `pr-comments:<url>` delegation",
            "original PR owner from the `pr: <pane_id> <url>` line",
            "rebase/merge the current default branch",
            "resolve conflicts",
            "rerun the task's verification",
            "push the same branch",
            "subtask `status: revising`",
            "not already `revising` / `in_progress`",
            "avoid duplicate conflict delegations",
        ] {
            assert!(
                got.contains(needle),
                "missing Tech Lead conflict-routing text: {needle}"
            );
        }
    }
    ...
    #[test]
    fn tech_lead_prompt_owns_worker_pr_recording() {
        let got = TECH_LEAD_DEADLOOP_PROMPT;
        for needle in [
            "worker creates the PR itself once the Reviewer approves",
            "append a new line `pr: <pane_id> <url>`",
            "When every contributing worker has its PR line, flip the global's `status:` to `pr_open`",
            "You don't run `gh pr create` yourself",
        ] {
            assert!(
                got.contains(needle),
                "missing Tech Lead PR recording text: {needle}"
            );
        }
    }
