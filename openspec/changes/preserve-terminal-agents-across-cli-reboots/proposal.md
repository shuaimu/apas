...
- Keep transport recovery automatic and internal: the project CLI rebuilds its own server WebSocket with bounded backoff, without stopping, respawning, or changing any pane process, and without a user-facing control.
...
- Keep full **Reboot CLI** for binary replacement as the only project lifecycle control, with its disruptive/fallback behavior made visible to the user.
...
- `cli-lifecycle-control`: Defines automatic transport recovery, full CLI replacement, user-visible lifecycle states, authorization, compatibility, and fallback behavior.
...
- `pane-toolbar`: Presents a single reboot control and communicates whether a full reboot can preserve the active terminal runtime.
...
- Extends shared CLI/server/web protocol messages and capabilities for reboot requests and progress/results while remaining compatible with older clients, which includes tolerating a lifecycle operation an older web client still sends.
