...
- [Thread count in one process] → Measured rather than estimated, and it comes out the other way. On zoo-005: the runtime baseline is 66 threads with no projects, and a live project adds only 11–13 — the ~30 figure came from counting `thread::spawn` call sites, not all of which execute. Because every separate process pays that 66-thread baseline, four projects today cost 377 threads across five processes and are projected at ~113 merged. Merging reduces thread count substantially rather than multiplying it. Memory is dominated by what a project is doing (2.6–23.8 MiB observed), so merging saves roughly one baseline per project and little else.
