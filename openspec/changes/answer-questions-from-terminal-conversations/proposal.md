...
- No server change: `WebToServer::AnswerQuestion` and `ServerToCli::AnswerQuestion` already carry everything needed.
- `packages/web`: the question card is reused as-is; the conversation view stops treating a terminal pane as read-only for questions.
